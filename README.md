...
- 이번 프로젝트에서 RxSwift를 사용하며 반응형 프로그래밍의 플로우를 경험하고 이해할 수 있었습니다. 또한, 이전 프로젝트부터 적용한 MVVM에 Input/Output pattern을 도입하면서 화면에서 일어나는 이벤트는 Input으로, 이벤트에 따라 바뀌는 결과는 output으로 구분하여 데이터 흐름을 단방향으로 개선한 구조로 관리할 수 있었습니다. 아쉬웠던 점은 RxSwift를 최대한 활용한 프로젝트를 목표로 하였으나, 컬렉션뷰나 테이블뷰를 중첩해서 사용하는 복잡한 UI일 경우 기존 방식으로 로직을 구현한 것처럼 반응형으로 구현하지 못한 view들이 존재한다는 점입니다. 이 부분은 RxSwift를 더 공부하여, 이후 프로젝트부터는 반응형 프로그래밍 구조로 구현할 예정입니다.
